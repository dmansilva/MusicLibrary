	...
	public int compare(Song user1, Song user2) {
		...
		if (user1.getArtist().equals(user2.getArtist())){          // if artists are the same
			...
			if (user1.getTitle().equals(user2.getTitle())) {      // if titles are the same
				...
				return user1.getTitle().compareTo(user2.getTitle());         // if titles aren't the same, return the comparison of titles
			}
		} else {
			
			return user1.getArtist().compareTo(user2.getArtist());			// if artists aren't the same, return the comparison of artists
		}
	}
